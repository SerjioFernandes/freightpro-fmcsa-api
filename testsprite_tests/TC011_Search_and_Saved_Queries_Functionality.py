...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login to authenticate with provided credentials.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=No matching loads found for the advanced search criteria').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The advanced search did not return accurate results or saved queries functionality failed as per the test plan.")
