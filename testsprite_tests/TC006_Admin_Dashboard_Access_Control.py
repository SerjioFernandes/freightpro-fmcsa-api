...
async def run_test():
    ...
    try:
        ...
        # -> Login as broker user with provided credentials to test access restriction.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Click Sign In button to submit broker login form
        elem = frame.locator('xpath=html/body/div/div[2]/main/div/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click Sign In button to attempt login as broker user and verify access restrictions.
        frame = context.pages[-1]
        # Click Sign In button to submit broker user login form
        elem = frame.locator('xpath=html/body/div/div[2]/main/div/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click Sign In button to attempt login as broker user and verify access restriction.
        frame = context.pages[-1]
        # Click Sign In button to submit broker user login form
        elem = frame.locator('xpath=html/body/div/div[2]/main/div/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        try:
            ...
            raise AssertionError('Test case failed: Access to admin command center is not properly restricted. Non-admin users should be denied access, but the test plan execution failed to verify this.')
